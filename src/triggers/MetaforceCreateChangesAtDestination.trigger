...
//                    Org_Id__c        = changeSetMap.get(updatedChangeJunction.Change_Set__c).Destination_Org_Id__c,
    ...
    {
        if (String.valueOf(e).contains('The selected environment is no longer in use and cannot accept new changes.'))
        {
            for (Change_Junction__c changeJunction : Trigger.New)
            {
                changeJunction.addError('The change cannot be deployed because the destination environment has been refreshed or deleted.');
            }
        }
    }
    catch (Exception e)
